...
void handle_resize() {
    ...
    // Calcular los límites de la vista del jugador en base a la posición actual
    int32_t player_chunk_x = player->ep_limit_nw.pos_x / 16;
    int32_t player_chunk_y = player->ep_limit_nw.pos_y / 16;
    int32_t view_distance_chunks = 5; // Distancia visible en "chunks"

    // Establecer límites de la zona visible del jugador
    player->ep_limit_nw.pos_x = (player_chunk_x - view_distance_chunks) * 16;
    player->ep_limit_nw.pos_y = (player_chunk_y - view_distance_chunks) * 16;
    player->ep_limit_se.pos_x = (player_chunk_x + view_distance_chunks) * 16;
    player->ep_limit_se.pos_y = (player_chunk_y + view_distance_chunks) * 16;
}void draw_ui(int ticks_elapsed, double tps, char current_c) {
	wclear(info_win);
