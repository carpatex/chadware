...
int main() {
	...
	while (game) {
		...
		//
		n_events_input = 1;
		chunk_event->output = client_chunks;
		chunk_event->entity_index_id = 0; // 0 is the first and only entity and player
...
void initialize_memory() {
	...
	if (!client_chunks) {
		perror("Failed to allocate client chunk memory.");
		exit(1);
	}
...
void draw_game_content(int current_tick) { // draw_game_content should use client_chunks, not lchunk_ptr
